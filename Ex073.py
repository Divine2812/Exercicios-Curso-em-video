...
print(f"\033[35mOs 5 primeiros colocados são: \033[36m{times[0]}", end= '')
for c in range (1,4):
    print(', ',end= str(times[c]))
print(" e " '', end=times[4] + '.\033[m')
print(f"\n\033[35mOs 4 últimos colocados são: \033[31m{str(times[-4])}", end= '')
for a in range (-3, -1):
    print(', ', end= str(times[a]))
print(" e " '', end=times[-1] + '.\033[m')
ui = (sorted(times))[1:-1]#.replace("'", "")
#last = str(i[-1]).replace("'", "")
#resto = str(i[0 : -2]).replace("'[]", "")
ui = sorted(times)
print(f"\n\033[35mA lista de times em ordem alfabética: \033[36m{ui[0]}",end= '')
for u in range (1, 18):
    print(', ', end= str(ui[u]))
